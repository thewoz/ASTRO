...
namespace ast2Body
{
	...
	* --------------------------------------------------------------------------- */
	...
	{
		double small, hbar[3], nbar[3], magr, magv, magn, ebar[3], sme, rdotv, temp, c1, hk, magh;
    ...
    double twopi = M_PI;
		...
		if (magh > small)
		{
			...
			//		   strcpy(typeorbit, 2 * sizeof(char), "ei");
			//#else
			//		   strcpy(typeorbit, "ei");
			//#endif
			typeorbit = 1;
			...
			if (ecc < small)
			{
				...
				{
					//#ifdef _MSC_VER
					//				   strcpy(typeorbit, sizeof(typeorbit), "ce");
					//#else
					//				   strcpy(typeorbit, "ce");
					//#endif
					typeorbit = 2;
				...
				else
				{
					// --------------  circular inclined ---------------
					//#ifdef _MSC_VER
					//				   strcpy(typeorbit, sizeof(typeorbit), "ci");
					//#else
					//				   strcpy(typeorbit, "ci");
					//#endif
					typeorbit = 3;
			...
			else
			{
				...
				{
					//#ifdef _MSC_VER
					//				   strcpy(typeorbit, sizeof(typeorbit), "ee");
					//#else
					//				   strcpy(typeorbit, "ee");
					//#endif
					typeorbit = 4;
	...
	*    vallado 2013 : 108
		...
		)
	{
		...
		p = a * (1.0 - ecc * ecc);
		incl = M_PI * ((1.0 - fr) * 0.5) + 2.0 * fr * atan(sqrt(chi * chi + psi * psi));
		raan = atan2(chi, psi);
	...
	*    vallado       2013, 93, alg 8, ex 2-4
		...
		)
	{
		...
		if (fabs(dtseco) > small)
		{
			...
			if (alpha >= small)
			{
				period = (M_PI*2) * sqrt(fabs(a * a * a) / mu);
	...
	* --------------------------------------------------------------------------- */
    ...
    double twopi = M_PI * 2;
		...
		if (fabs(temp) < small)
			rtasc = astMath::sgn(recef[2]) * M_PI * 0.5;
		else
			rtasc = atan2(recef[1], recef[0]);

		lon = rtasc;
		if (fabs(lon) >= M_PI)   // mod it ?
		{
			if (lon < 0.0)
				lon = twopi + lon;
			else
		...
		if ((M_PI * 0.5 - fabs(latgd)) > M_PI / 180.0)  // 1 deg
