...
namespace astIOD 
{
   ...
   )
  {
    ...
    if ((df == 'r') && (nrev > 0))
    {
      ...
      while ((fabs(xn - x) >= small) && (loops <= 20))
      {
        ...
        temp = 1.0 / (2.0*(L - x*x));
        temp1 = sqrt(x);
        temp2 = (nrev*M_PI*0.5 + atan(temp1)) / temp1;
      ...
      fprintf(outfile,"high v1t %16.8lf %16.8lf %16.8lf %16.8f\n", v1t[0], v1t[1], v1t[2], astMath::mag(v1t));
    }
    else
    {
      // standard processing
      ...
      while ((fabs(xn - x) >= small) && (loops <= 30))
      {
        if (nrev > 0)
        {
          ...
          temp1 = (nrev*M_PI*0.5 + atan(sqrt(x))) / sqrt(x);
   ...
   )
  {
    ...
    if (nrev == 0)
    {
      upper = 4.0 * M_PI * M_PI;  // could be negative infinity for all cases
      lower = -4.0 * M_PI * M_PI; // allow hyperbolic and parabolic solutions
    }
    else
    {
      lower = 4.0 * nrev * nrev * M_PI * M_PI;
      upper = 4.0 * (nrev + 1.0) * (nrev + 1.0) * M_PI * M_PI;
    }
   ...
   )
  {
    ...
    outfile = fopen("D:/Codes/LIBRARY/CPP/Libraries/astIOD/testastIOD.out", "w");
