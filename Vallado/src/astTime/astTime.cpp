  ...
  *    jdtdb       - julian date of tdb             days from 4713 bc
	...
	* --------------------------------------------------------------------------- */
	...
	void    convtime
		...
		double& tt, double& ttt, double& jdtt, double& jdttFrac, double& tdb, double& ttdb,
		double& jdtdb, double& jdtdbFrac, double& tcg, double&  jdtcg, double& jdtcgFrac,
     double&  tcb, double&  jdtcb,double& jdtcbFrac
		)
	{
		double deg2rad, jd, jdFrac, sectemp;
		int localhr, hrtemp, mintemp;

		deg2rad = M_PI / 180.0;

		// ------------------------  implementation   ------------------
		astTime::jday(year, mon, day, hr+timezone, min, sec, jd, jdFrac);
    //mjd  = jd+jdFrac - 2400000.5; //non usate
    //mfme = hr*60.0 + min + sec/60.0; //non usate
    ...
    double jdtai, jdtaiFrac;
		tai = utc + dat;
    astTime::hms_sec(hrtemp, mintemp, sectemp, eFrom, tai);
    astTime::jday(year, mon, day, hrtemp, mintemp, sectemp, jdtai, jdtaiFrac);
    ...
    astTime::hms_sec(hrtemp, mintemp, sectemp, eFrom, tcg);
    astTime::jday(year, mon, day, hrtemp, mintemp, sectemp, jdtcg, jdtcgFrac);

    double tcbmtdb = 1.55051976772e-8*(jdtai+jdtaiFrac - 2443144.5)*86400.0;  // sec, value for de405 AAS 05-352 (10) and IERS TN (104)?
    tcb = tdb + tcbmtdb;
    astTime::hms_sec(hrtemp, mintemp, sectemp, eFrom, tcb);
    astTime::jday(year, mon, day, hrtemp, mintemp, sectemp, jdtcb, jdtcbFrac);
  ...
  (
   int year, int mon, int day, int hr, int min, double sec, int timezone,
   double dut1, int dat, double& jdut1, double & jdut1Frac, double& ttt)
  {
    double deg2rad, jd, jdFrac, sectemp;
    int localhr, hrtemp, mintemp;
    
    double utc, ut1, tut1, tai, tt, jdtt, jdttFrac;
